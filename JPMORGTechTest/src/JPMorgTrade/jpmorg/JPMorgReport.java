 ...
 * JPMorgReport.java
 * 
 * Copyright 2017 Sankareswari R
 * 
 * 
 */

package JPMorgTrade.jpmorg;
...
public class JPMorgReport {
  ...
  public static void main(String args[]) {
      ...
      try {
        ...
        Map<Date, Double> sumIncomingByDate = jpMorg.settlementIncomingReport(jpMorg.getDataListSell());
        ...
        Map<Date, Double> sumOutgoingByDate = jpMorg.settlementOutgoingReport(jpMorg.getDataListBuy());
        sb = new StringBuilder();
  ...
  public Map<Date, Double> settlementIncomingReport(List<InnerDataRow> idr) {    
    Map<Date, Double> sumIncomingByDate = idr.stream().collect(
                                                               Collectors.groupingBy(InnerDataRow::getActualSetlDate, 
                                                                                     Collectors.summingDouble(InnerDataRow::getUsdAmount)));
    return sumIncomingByDate;
  }
  
  // Generate outgoing settlement report
  public Map<Date, Double> settlementOutgoingReport(List<InnerDataRow> idr) {    
    Map<Date, Double> sumOutgoingByDate = idr.stream().collect(
                                                               Collectors.groupingBy(InnerDataRow::getActualSetlDate, 
                                                                                     Collectors.summingDouble(InnerDataRow::getUsdAmount)));
    return sumOutgoingByDate;
  }
  ...
  private double calculateAmount (String pricePerUnit, String units, String agreedFx) 
    ...
    return Double.valueOf(df.format(Double.parseDouble(pricePerUnit) * Double.parseDouble(units) * Double.parseDouble(agreedFx)));
  }
